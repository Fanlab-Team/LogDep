...
from .utils import load_pickle, save_pickle, load_tuple_list, cache_to_file, read_json_file, count_templates
...
def save_results_to_csv(log_file, template_file, cache_file, output_file, output_template_file):
    ...
    #在解析结果的Template文件里加一列出现次数
    count_templates(output_file,output_template_file)
...
class LogParser:
    def __init__(self, log_format, indir='./', outdir='./result/', rex=[],
                 data_type='2k', shot=0, example_size=0, model="deepseek-chat", selection_method="LILAC"):
        self.path = indir
        self.df_log = None
    ...
    def parse(self, logName):
        ...
        dataset_name = os.path.splitext(os.path.basename(logName))[0].split('_')[0]
