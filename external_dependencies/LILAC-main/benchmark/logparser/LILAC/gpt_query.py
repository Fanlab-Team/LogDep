...
api_base = os.environ.get("LILAC_API_BASE_URL", "https://api.deepseek.com").strip()
api_key = os.environ.get("DEEPSEEK_API_KEY", "").strip()
if not api_key:
    api_base, api_key = get_openai_key('../../openai_key.txt')
...
def infer_llm(instruction, exemplars, query, log_message, model='deepseek-chat', temperature=0.0, max_tokens=2048):
...
def get_response_from_openai_key(query, examples=[], model='deepseek-chat', temperature=0.0):
    # Prompt-1
...
def query_template_from_gpt(log_message, examples=[], model='deepseek-chat'):
...
def query_template_from_gpt_with_check(log_message, regs_common=[], examples=[], model="deepseek-chat"):
    ...
    if len(template) == 0 or flag == False:
        ...
        if flag:
            ...
            if tree.match_event(log_message)[0] == "NoMatch":
                ...
                print(log_message)
                print(
                    "ChatGPT template wrong: cannot match itself! And the wrong template is : ")
                print(template)
                print("==========================================================")
            else:
                return template, True
