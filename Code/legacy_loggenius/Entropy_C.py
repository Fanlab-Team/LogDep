...
def log_entropy_C(sentences,threshold,dataset,priority_filter='all',max_llm_calls=None):
    ...
    for idx, selected in zip(template_indices, selected_mask):
        if selected:
    ...
    with open('../log_after_entropy/'+dataset+'_expand_reason.csv', 'w', newline='', encoding='gbk') as csvfile:
        ...
        writer.writerow(['represent_index', 'origin_sentence', 'template', 'template_size',
                         'variable_count', 'reason', 'priority', 'selected_for_llm'])

        for idx, reason_info, selected in zip(template_indices, selection_reasons, selected_mask):
