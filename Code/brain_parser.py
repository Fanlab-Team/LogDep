        ...
        for pr in parse_result[key]:
            ...
            while i < len(sort):
                ...
                if bool(re.search(r"/", this)):
                    ...
                    continue
                #能够识别普通数字的同时也能够识别负数
                if this.isdigit() or re.match(r'^-\d+$', this):
                    template.append('<*>')
                    i+=1
                    continue
                if bool('<*>' in this):
...
def parse_E(sentences,filter,dataset,threshold,delimiter,tag,starttime,efficiency):
    ...
    for key in wordlist.keys():
        ...
        for k in root_set_detail:
            choose_flag=1
            for logs in root_set_detail[k]:
                c=0
                loglines+=1
                while c <len(logs)-1:
                    if logs[c][0]==k[0]:
                        if "<*>" in logs[c][1] and logs[c][1] not in template[logs[len(logs)-1][0]]:
                            choose_flag=0
                    if choose_flag==0:
                        break
                    c+=1
    ...
    if dataset in ['HDFS', 'Hadoop', 'Spark', 'Zookeeper', 'BGL', 'HPC', 'Thunderbird', 'Windows', 'Linux',
                   'Android', 'HealthApp', 'Apache', 'OpenSSH', 'OpenStack', 'Mac', 'Proxifier']:
